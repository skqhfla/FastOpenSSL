...
int new_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t inl);
...
int encrypt(FILE *in_file, FILE *out_file, const unsigned char *key, FILE *error_stream)
{
    ...
    // 암호화 알고리즘 객체 생성 (AES-256 GCM)
    cipher = EVP_CIPHER_meth_new(NID_aes_256_gcm, 16, 32);
    if (!cipher)
    {
        fprintf(stderr, "Failed to create EVP_CIPHER\n");
        return 1;
    }
